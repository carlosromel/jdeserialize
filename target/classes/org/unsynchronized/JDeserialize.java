...
/**
 * The main user-facing class for the jdeserialize library. Also the
 * implementation of the command-line tool.<br/>
 * <br/>
 * Library:<br/>
 * <br/>
 * The jdeserialize class parses the stream (method run()). From there, call the
 * getContent() method to get an itemized list of all items written to the
 * stream, or getHandleMaps() to get a list of all handle->content maps
 * generated during parsing. The objects are generally instances that implement
 * the interface "content"; see the documentation of various implementors to get
 * more information about the inner representations.<br/>
 * <br/>
 * To enable debugging on stdout, use the enableDebug() or disableDebug()
 * options.   <br/>
 * <br/>
 * <br/>
 * Command-line tool:   <br/>
 * <br/>
 * The tool reads in a set of files and generates configurable output on stdout.
 * The primary output consists of three separate stages. The first stage is a
 * textual description of every piece of content in the stream, in the order it
 * was written. There is generally a one-to-one mapping between
 * ObjectOutputStream.writeXXX() calls and items printed in the first stage. The
 * first stage may be suppressed with the -nocontent command-line option. <br/>
 * <br/>
 * The second stage is a list of every class declaration serialized in the file.
 * These are formatted as normal Java language class declarations. Several
 * options are available to govern this stage, including -filter, -showarrays,
 * -noclasses, and -fixnames. <br/>
 * <br/>
 * The third stage is a dump of every instance embedded inside the stream,
 * including textual descriptions of field values. This is useful for casual
 * viewing of class data. To suppress this stage, use -noinstances. <br/>
 * <br/>
 * You can also get debugging information generated during the parse phase by
 * supplying -debug.
 * <br/>
 * The data from block data objects can be extracted with the -blockdata <file>
 * option. Additionally, a manifest describing the size of each individual block
 * can be generated with the -blockdatamanifest <file> option.
 * <br/>
 * References: <br/>
...
public class JDeserialize {
     ...
     * Each item generally corresponds to an invocation of an ObjectOutputStream
     * writeXXX() method. A notable exception is the class exceptionstate, which
     * represents an embedded exception that was caught during serialization.
     * </p>
     *
     * <p>
     * See the various implementors of content to get information about what
     * data is available.
     * </p>
     *
     * <p>
     * Entries in the list may be null, because it's perfectly legitimate to
     * write a null reference to the stream.
     * </p>
     *
    ...
    /**
     * <p>
     * Return a list of Maps containing every object with a handle. The keys are
     * integers -- the handles themselves -- and the values are instances of
     * type content.
     * </p>
     *
     * <p>
     * Although there is only one map active at a given point, a stream may have
     * multiple logical maps: when a reset happens (indicated by TC_RESET), the
    ...
    /**
     * Suitably escapes non-printable-ASCII characters (and doublequotes) for
     * use in a Java string literal.
     *
    ...
    /**
     * "Fix" the given name by transforming illegal characters, such that the
     * end result is a legal Java identifier that is not a keyword. If the
     * string is modified at all, the result will be prepended with "$__".
     *
     * @param name the name to be transformed
     * @return the unmodified string if it is legal, otherwise a
     * legal-identifier version
     */
    ...
    /**
     * Read the content of a thrown exception object. According to the spec,
     * this must be an object of type Throwable. Although the Sun JDK always
     * appears to provide enough information about the hierarchy to reach all
     * the way back to java.lang.Throwable, it's unclear whether this is
     * actually a requirement. From my reading, it's possible that some other
     * ObjectOutputStream implementations may leave some gaps in the hierarchy,
     * forcing this app to hit the classloader. To avoid this, we merely ensure
     * that the written object is indeed an instance; ensuring that the object
     * is indeed a Throwable is an exercise left to the user.
     */
    ...
    /**
     * <p>
     * Read the next object corresponding to the spec grammar rule "content",
     * and return an object of type content.
     * </p>
     *
     * <p>
     * Usually, there is a 1:1 mapping of content items and returned instances.
     * The one case where this isn't true is when an exception is embedded
     * inside another object. When this is encountered, only the serialized
     * exception object is returned; it's up to the caller to backtrack in order
     * to gather any data from the object that was being serialized when the
     * exception was thrown.
     * </p>
     *
     * @param tc the last byte read from the stream; it must be one of the TC_*
     * values within ObjectStreamConstants.
     *
     * @param dis the DataInputStream to read from
     * @param blockdata whether or not to read TC_BLOCKDATA (this is the
     * difference between spec rules "object" and "content").
     ...
     * this object's content and handle maps with data about the objects in the
     * stream.
     * </p>
     *
     * <p>
     * If shouldConnect is true, then jdeserialize will attempt to identify
     * member classes by their names according to the details laid out in the
     * Inner Classes Specification. If it finds one, it will set the classdesc's
     * flag indicating that it is an member class, and it will create a
     * reference in its enclosing class.
     * </p>
     *
     * @param is an open InputStream on a serialized stream of data
     * @param shouldConnect true if jdeserialize should attempt to identify and
     * connect member classes with their enclosing classes
     *
     * Also see the <pre>connectMemberClasses</pre> method for more information
     * on the member-class-detection algorithm.
     */
    ...
    /**
     * <p>
     * Connects member classes according to the rules specified by the JDK 1.1
     ...
     * classdesc, and the isStaticMemberClass value in classdesc where
     * necessary.
     * </p>
     *
     * <p>
     * A word on static classes: serializing a static member class S doesn't
     * inherently require serialization of its parent class P. Unlike inner
     * classes, S doesn't retain an instance of P, and therefore P's class
     * description doesn't need to be written. In these cases, if parent classes
     * can be found, their static member classes will be connected; but if they
     * can't be found, the names will not be changed and no ValidityException
     * will be thrown.
     * </p>
     *
    ...
    public void connectMemberClasses() throws IOException {
        ...
        for (ClassDesc cd : classes.values()) {
            for (ClassDesc ncd : newnames.keySet()) {
                ...
                for (ClassDesc cdx : classes.values()) {
                    if (cdx.classtype == ClassDescType.PROXYCLASS) {
                        continue;
                    }
                    for (Field f : cdx.fields) {
    ...
    /**
     * Decodes a class name according to the field-descriptor format in the jvm
     * spec, section 4.3.2.
     *
     * @param fdesc name in field-descriptor format (Lfoo/bar/baz;)
     * @param convertSlashes true iff slashes should be replaced with periods
     * (true for "real" field-descriptor format; false for names in classdesc)
